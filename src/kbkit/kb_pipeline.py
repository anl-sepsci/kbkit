...
class KBPipeline:
    ...
    def __init__(
        ...
        pure_component_path: Optional[str] = None,
        base_systems: Optional[list[str]] = None,
        pure_component_systems: Optional[list[str]] = None,
        ...
        self.kb = KBThermo(
            ...
            cation_list=cation_list if cation_list is not None else [],
            anion_list=anion_list if anion_list is not None else [],
        )

    def run(self, energy_units: str = "kJ/mol") -> None:
        r"""
    ...
    def to_dict(self, energy_units: str = "kJ/mol") -> dict[str, np.ndarray]:
        r"""
    ...
    def to_dataframe(self, name: str = "", energy_units: str = "kJ/mol") -> pd.DataFrame:
        r"""
        ...
        if name.lower() in ("all", ""):
            for key in thermo_dict:
                ...
                elif thermo_dict[key].ndim > 1:
                    _dict.update(
                        {f"{key}_{mol}": thermo_dict[key][:, i] for i, mol in enumerate(self.kb.unique_molecules)}
                    )

        # only add properties with key in thermo_keys to dataframe
        elif any(x in name.lower() for x in ["thermo", "gibbs"]):
        ...
        elif any(x in name.lower() for x in ["saxs", "i0", "io"]):
        ...
        elif (match := next((k for k in (name.lower(), name.upper()) if k in thermo_dict), None)) is not None:
            if thermo_dict[match].ndim == 1:
                _dict[match] = thermo_dict[match]
            elif thermo_dict[match].ndim > 1:
                _dict.update(
                    {f"{match}_{mol}": thermo_dict[match][:, i] for i, mol in enumerate(self.kb.unique_molecules)}
                )
            else:
        ...
        return pd.DataFrame(_dict)  # create pandas.dataframe object
