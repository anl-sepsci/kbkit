...
def to_float(val: float | list[float] | tuple[float, ...]) -> float:
    """
    Convert value to float; if list/tuple provided return float of first value.

    Parameters
    ----------
    val: float, list, tuple
        Value to return float of.
...
def format_unit_str(text: str | Quantity) -> str:
...
def generate_mol_frac_matrix(n_components: int) -> NDArray[np.float64]:
    """Generate a matrix of all possible mol fraction compositions for a mixture with a specified number of components.

    In this matrix, each composition sums to 1.0 and each component's mole fraction is chosen from a discrete set of values.
    ...
    if not isinstance(n_components, int) or n_components not in spacing_map:
