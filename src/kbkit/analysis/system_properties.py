...
from kbkit.parsers import EdrFileParser, GroFileParser, TopFileParser
...
class SystemProperties:
    ...
    Attributes
    ----------
    edr_paths: list[EdrFileParser]
        List of paths to GROMACS energy files.
    top_paths: list[TopFileParser]
        List of paths to GROMACS topology files.
    gro_paths: list[GroFileParser]
    ...
    def __init__(
        ...
        if system_path is not None:
            self.system_path = validate_path(system_path)
    ...
    @cached_property
    def energy(self) -> list[EdrFileParser]:
        """list[EdrFileParser]: Setup EDR file parsers for all files in ``edr_paths``."""
        if len(self.edr_paths) == 0:
            raise FileNotFoundError("Energy file(s) do not exist!")

        return [EdrFileParser(Path(fpath)) for fpath in self.edr_paths]

    @cached_property
    def topology(self) -> GroFileParser | TopFileParser:
        """GroFileParser | TopFileParser: Setup GRO/TOP file parser."""
        # prioritize GRO files -- they contain electron info as well
        if len(self.gro_paths) > 0:
            for file in self.gro_paths:
                gro = GroFileParser(file)
                if any(len(mol) > 1 for mol in gro.molecules):
                    return gro
        # if gro file(s) are invalid; use topology file
        elif len(self.top_paths) > 0:
            for file in self.top_paths:
                return TopFileParser(file)
        else:
    ...
    def get(
        ...
        if avg or prop in EdrFileParser.FLUCT_PROPS:
