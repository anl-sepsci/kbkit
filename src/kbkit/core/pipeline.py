...
if TYPE_CHECKING:
    from kbkit.plotter import KBIAnalysisPlotter, ThermoPlotter, TimeseriesPlotter
...
class Pipeline:
    ...
    def kbi_res(self) -> PropertyResult:
        ...
        return self.systems.properties.kbi(
            units="cm^3/mol",
    ...
    def results(self) -> dict[str, PropertyResult]:
        ...
        for prop, units in self.systems.units.items():
            if "time" in prop.lower():
                continue
            res[f"simulated_{prop.lower().replace('-', '_')}"] = self.systems.properties.simulated_property(
                name=prop, units=units, avg=True
            )
            res[f"ideal_{prop.lower().replace('-', '_')}"] = self.systems.properties.ideal_property(
                name=prop, units=units, avg=True
            )
            res[f"excess_{prop.lower().replace('-', '_')}"] = self.systems.properties.excess_property(
                name=prop, units=units, avg=True
            )
        return res
    ...
    def kbi_plotter(self) -> "KBIAnalysisPlotter":
        ...
        return self.systems.kbi_plotter(kbi=self.kbi_res, molecule_map=self.molecule_map)
