...
def mock_edr_parser():
    ...
    mock_edr.units = {"Temperature": "K", "Pressure": "bar", "Volume": "nm^3", "molar-volume": "cm^3/mol"}
...
class TestSystemPropertiesInitialization:
    ...
    def test_init_with_system_path(self, mock_find_files, tmp_path):
        ...
        mock_find_files.side_effect = lambda **kwargs: {".edr": [edr_file], ".gro": [gro_file], ".top": [top_file]}.get(
            kwargs.get("suffix"), []
        )
    ...
    def test_init_with_custom_start_time(self, mock_find_files, tmp_path):
        ...
        mock_find_files.side_effect = lambda **kwargs: {".edr": [edr_file], ".gro": [gro_file], ".top": []}.get(
            kwargs.get("suffix"), []
        )
...
class TestSystemPropertiesFindFiles:
    ...
    def test_find_files_with_include_filter(self, tmp_path):
        ...
        with patch("kbkit.systems.properties.validate_path", return_value=tmp_path):
            files = SystemProperties.find_files(suffix=".edr", system_path=str(tmp_path), include="npt")
    ...
    def test_find_files_with_exclude_filter(self, tmp_path):
        ...
        with patch("kbkit.systems.properties.validate_path", return_value=tmp_path):
            files = SystemProperties.find_files(suffix=".edr", system_path=str(tmp_path), exclude=["init", "eq"])
...
class TestSystemPropertiesTopologyProperty:
    ...
    def test_topology_raises_when_no_files(self, mock_find_files, tmp_path):
        ...
        mock_find_files.side_effect = lambda **kwargs: {".edr": [tmp_path / "system.edr"], ".gro": [], ".top": []}.get(
            kwargs.get("suffix"), []
        )
...
class TestSystemPropertiesTimeseriesPlotter:
    ...
    @patch("kbkit.systems.properties.GroParser")
    @patch("kbkit.systems.properties.SystemProperties.find_files")
    def test_timeseries_plotter_creates_plotter(self, mock_find_files, mock_gro_class, mock_plotter_class, tmp_path):
    ...
    @patch("kbkit.systems.properties.GroParser")
    @patch("kbkit.systems.properties.SystemProperties.find_files")
    def test_timeseries_plotter_default_start_time(self, mock_find_files, mock_gro_class, mock_plotter_class, tmp_path):
