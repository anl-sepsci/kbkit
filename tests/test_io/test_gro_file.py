...
import MDAnalysis
from MDAnalysis.core.groups import ResidueGroup, AtomGroup
...
class TestAtomCounts:
    ...
    def test_atom_counts_values_are_integers(self, sample_gro_file):
        ...
        for resname, atoms in atom_counts.items():
...
class TestEdgeCases:
    ...
    def test_empty_gro_file(self, tmp_path):
        """Test handling of empty .gro file."""
        gro_file = tmp_path / "empty.gro"
        gro_file.write_text("")
        
        with pytest.raises(Exception):  # MDAnalysis will raise an error
            parser = GroParser(str(gro_file))

    def test_malformed_gro_file(self, tmp_path):
        """Test handling of malformed .gro file."""
        gro_content = """Malformed file
    invalid content
"""
        gro_file = tmp_path / "malformed.gro"
        gro_file.write_text(gro_content)
    ...
    def test_single_molecule(self, tmp_path):
        """Test system with single molecule."""
        gro_content = """Single molecule
    3
    1WAT     OW    1   0.126   0.126   0.126
    1WAT    HW1    2   0.190   0.126   0.126
    1WAT    HW2    3   0.062   0.126   0.126
...
"""
        gro_file = tmp_path / "single.gro"
        gro_file.write_text(gro_content)
        
        parser = GroParser(str(gro_file))
        assert parser.total_molecules == 1
        assert len(parser.molecules) == 1
...
class TestEdgeCases:
    ...
    def test_case_insensitivity(self):
        ...
        for lower, proper, name in test_cases:
...
class TestIntegration:
    ...
    def test_truncation_behavior(self):
        ...
        for long_name, expected_valid in test_cases:
